...
const store = async (req, res, next) => {
    try{
        ...
        if(payload.category){
            let category = 
                await Category
                .findOne({jenis: {$regex: payload.category, $options: "i"}});
            if(category){
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    if(err && err.jenis == "ValidationError"){
                        return res.json({
                            error: 1,
                            message: err.message,
                            fields: err.errors
...
const index = async (req, res, next) => {
    try {
        ...
        if(q.length) {
            criteria = {
                ...criteria,
                jenis: {$regex: `${q}`, $options: ""}
            }
        }

        if(category.length) {
            let categoryResult = await Category.findOne({jenis: {$regex: `${category}`}, $options: "i"});
        ...
        if(tags.length) {
            let tagsResult = await Tag.find({jenis: {$in: tags}});
