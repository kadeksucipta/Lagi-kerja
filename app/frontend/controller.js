        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new Frontend({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new Frontend(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await Frontend.findById(id);
                    ...
                    product = await Frontend.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await Frontend.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await Frontend.find().countDocuments();

        let product = await Frontend
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await Frontend.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
