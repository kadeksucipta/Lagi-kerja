        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new Java({...payload, image_url: filename})
                    await product.save()
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await Java.findById(id);
                    ...
                    product = await Java.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await Java.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await Java.find().countDocuments();

        let product = await Java
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await Java.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
