        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new DataEntry({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new DataEntry(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await DataEntry.findById(id);
                    ...
                    product = await DataEntry.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await DataEntry.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await DataEntry.find().countDocuments();

        let product = await DataEntry
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await DataEntry.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
