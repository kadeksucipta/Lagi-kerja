        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new Software({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new Software(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await Software.findById(id);
                    ...
                    product = await Software.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await Software.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await Software.find().countDocuments();

        let product = await Software
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await Software.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
