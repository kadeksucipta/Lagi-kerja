...
const DataEnginer = require("./model");
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new DataEnginer({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new DataEnginer(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await DataEnginer.findById(id);
                    ...
                    product = await DataEnginer.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await DataEnginer.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await DataEnginer.find().countDocuments();

        let product = await DataEnginer
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await DataEnginer.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
