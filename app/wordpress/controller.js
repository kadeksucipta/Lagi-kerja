        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new WordPress({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new WordPress(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await WordPress.findById(id);
                    ...
                    product = await WordPress.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await WordPress.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await WordPress.find().countDocuments();

        let product = await WordPress
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await WordPress.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
