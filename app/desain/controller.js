        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new Desain({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new Desain(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await Desain.findById(id);
                    ...
                    product = await Desain.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await Desain.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await Desain.find().countDocuments();

        let product = await Desain
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await Desain.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
